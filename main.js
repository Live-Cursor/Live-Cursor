...
var ConfigSyncEngine = class {
  constructor(app, serverUrl, user, pass, workspace = "default-workspace") {
    this.app = app;
    this.serverUrl = serverUrl;
    this.user = user;
    this.pass = pass;
  ...
  async getRemoteManifest() {
    ...
    const res = await (0, import_obsidian.requestUrl)({ url, method: "GET" });
    if (res.status !== 200) throw new Error("Failed to fetch manifest");
    return res.json;
  }
  async uploadFile(relativePath, data, mtime) {
  ...
  async syncConfig() {
    new import_obsidian.Notice("Starting Config Sync...");
    ...
    try {
      ...
      for (const local of localFiles) {
        ...
        if (!remote || local.stat.mtime > remote.mtime + 2e3) {
      ...
      for (const [relPath, remote] of Object.entries(remoteManifest)) {
        ...
        if (!localStat || remote.mtime > localStat.mtime + 2e3) {
      ...
      new import_obsidian.Notice(`Config Sync Complete: Uploaded ${uploaded}, Downloaded ${downloaded}`);
    } catch (e) {
      new import_obsidian.Notice(`Config Sync Failed: ${e.message}`);
      console.error(e);
    }
  }
};
...
var LiveCursorPlugin = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
    this.activeSyncs = /* @__PURE__ */ new Map();
  ...
  async startDaemon() {
    try {
      ...
      try {
        ...
        new import_obsidian2.Notice("Local background daemon only supported on desktop environments.");
        return false;
      }
      ...
      new import_obsidian2.Notice(`Failed to launch background daemon: ${e.message}`);
      return false;
    }
  }
  stopDaemon() {
    if (this.daemonProcess) {
      ...
      this.daemonProcess = null;
      new import_obsidian2.Notice("Local Server Stopped.");
    }
  }
  onunload() {
    this.stopDaemon();
  ...
  syncFile(file) {
    ...
    const provider = new WebsocketProvider(this.settings.serverUrl, roomName, doc2, {
      connect: true,
      params: {
    ...
    this.activeSyncs.set(file.path, { doc: doc2, provider });
...
var LiveCursorSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    const daemonStatusText = this.plugin.daemonProcess ? "\u{1F7E2} Local background sync server is running on port 1234." : "\u{1F534} Local background sync server is offline.";
    const daemonSetting = new import_obsidian2.Setting(containerEl).setName("Local Background Server").setDesc(daemonStatusText);
    if (this.plugin.daemonProcess) {
      daemonSetting.addButton((btn) => btn.setButtonText("Stop Server").onClick(() => {
        this.plugin.stopDaemon();
        this.display();
      }));
    } else {
      daemonSetting.addButton((btn) => btn.setButtonText("Start Server").setCta().onClick(async () => {
        const started = await this.plugin.startDaemon();
        if (started) this.display();
      }));
    }
    new import_obsidian2.Setting(containerEl).setName("Option 1: Login Portal").setDesc("Configure credentials (Admin or User access levels).").addButton((btn) => btn.setButtonText("Open Portal").onClick(() => {
      new RoleSelectionModal(this.app, this.plugin, this).open();
    }));
    new import_obsidian2.Setting(containerEl).setName("Option 2: Server Connection").setDesc("Establish your server websocket gateway URL.").addButton((btn) => btn.setButtonText("Connect Server").onClick(() => {
      new ConnectionModal(this.app, this.plugin).open();
    }));
    new import_obsidian2.Setting(containerEl).setName("Option 3: Advanced Cloud Server").setDesc("Self-host a robust persistent database cluster using Docker.").addButton((btn) => btn.setButtonText("Generate Compose").onClick(() => {
      new CreateServerModal(this.app).open();
    }));
    new import_obsidian2.Setting(containerEl).setName("Option 4: Config Synchronization").setDesc("Synchronize plugins, themes, and workspace state (.obsidian folder) with the server.").addButton((button) => button.setButtonText("Sync Now").onClick(async () => {
    ...
    if (this.plugin.settings.username === "admin") {
      new import_obsidian2.Setting(containerEl).setName("Option 5: Admin Console").setDesc("View registered users, server resource consumption, and telemetry.").addButton((btn) => btn.setButtonText("Launch Dashboard").setCta().onClick(() => {
        new AdminConsoleModal(this.app, this.plugin).open();
      }));
    }
    containerEl.createEl("br");
    containerEl.createEl("h3", { text: "Collaborator Profile" });
    new import_obsidian2.Setting(containerEl).setName("Visual Nickname").setDesc("Your display name shown to other real-time editors in the vault.").addText((text2) => text2.setPlaceholder("Anonymous Editor").setValue(this.plugin.settings.nickname).onChange(async (val) => {
      this.plugin.settings.nickname = val;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Cursor Color").setDesc("The color of your cursor and editor selections shown to others.").addColorPicker((color) => color.setValue(this.plugin.settings.cursorColor).onChange(async (val) => {
      this.plugin.settings.cursorColor = val;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("br");
    containerEl.createEl("h3", { text: "System Integration" });
    new import_obsidian2.Setting(containerEl).setName("Developer Mode").setDesc("Unlock advanced integration layouts, telemetry configuration, and direct text parameters.").addToggle((toggle) => toggle.setValue(this.plugin.settings.devMode).onChange(async (value) => {
    ...
    if (this.plugin.settings.devMode) {
      ...
      new import_obsidian2.Setting(containerEl).setName("Direct Server URL").setDesc("Set connection URL without modal popups.").addText((text2) => text2.setValue(this.plugin.settings.serverUrl).onChange(async (val) => {
        this.plugin.settings.serverUrl = val;
        await this.plugin.saveSettings();
      }));
      new import_obsidian2.Setting(containerEl).setName("Direct Username").setDesc("Set active session username directly.").addText((text2) => text2.setValue(this.plugin.settings.username).onChange(async (val) => {
        this.plugin.settings.username = val;
        await this.plugin.saveSettings();
      }));
      new import_obsidian2.Setting(containerEl).setName("Direct Password").setDesc("Set active session password directly.").addText((text2) => text2.setValue(this.plugin.settings.passwordHash).onChange(async (val) => {
        this.plugin.settings.passwordHash = val;
        await this.plugin.saveSettings();
      }));
      new import_obsidian2.Setting(containerEl).setName("Workspace Identifier").setDesc("Set unique isolation key for configuration sync.").addText((text2) => text2.setValue(this.plugin.settings.workspaceName).onChange(async (val) => {
        this.plugin.settings.workspaceName = val;
        await this.plugin.saveSettings();
      }));
      new import_obsidian2.Setting(containerEl).setName("Debug Logging").setDesc("Print verbose synchronization telemetry in the developer console.").addToggle((toggle) => toggle.setValue(this.plugin.settings.debugLogging).onChange(async (val) => {
        this.plugin.settings.debugLogging = val;
        await this.plugin.saveSettings();
      }));
      new import_obsidian2.Setting(containerEl).setName("Auto-Sync On Load").setDesc("Automatically run configuration sync when Obsidian registers the plugin.").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoSyncOnLoad).onChange(async (val) => {
...
var RoleSelectionModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("Admin Portal").setDesc("Access full administrative configurations and mirrors.").addButton((btn) => btn.setButtonText("Login as Admin").setCta().onClick(() => {
      this.close();
      new LoginModal(this.app, this.plugin, "Admin", this.tab).open();
    }));
    new import_obsidian2.Setting(contentEl).setName("User Portal").setDesc("Standard collaborative real-time editing workspace.").addButton((btn) => btn.setButtonText("Login as User").onClick(() => {
      this.close();
...
var LoginModal = class extends import_obsidian2.Modal {
  constructor(app, plugin, role, tab) {
    super(app);
    this.plugin = plugin;
    this.role = role;
    this.tab = tab;
  ...
  async onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("Username").addText((text2) => text2.setValue(username).onChange((v) => username = v));
    new import_obsidian2.Setting(contentEl).setName("Password").addText((text2) => text2.setValue(password).onChange((v) => password = v));
    let actionText = "Login";
    if (this.role === "Admin") {
      try {
        let httpUrl = this.plugin.settings.serverUrl.replace(/^ws/, "http").replace(/\/sync\/?$/, "");
        const res = await (0, import_obsidian2.requestUrl)({ url: `${httpUrl}/api/admin-exists`, method: "GET" });
    ...
    new import_obsidian2.Setting(contentEl).addButton((btn) => btn.setButtonText(actionText).setCta().onClick(async () => {
      this.plugin.settings.username = username;
      this.plugin.settings.passwordHash = password;
      await this.plugin.saveSettings();
      this.tab.display();
      new import_obsidian2.Notice(`${this.role} credentials saved!`);
      this.close();
    }));
  }
  onClose() {
...
var ConnectionModal = class extends import_obsidian2.Modal {
  constructor(app, plugin) {
    super(app);
    this.plugin = plugin;
  }
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("Server URL").setDesc("e.g. ws://localhost:1234/sync").addText((text2) => text2.setValue(url).onChange((v) => url = v));
    new import_obsidian2.Setting(contentEl).addButton((btn) => btn.setButtonText("Save & Connect").setCta().onClick(async () => {
      this.plugin.settings.serverUrl = url;
      await this.plugin.saveSettings();
      new import_obsidian2.Notice("Server URL saved!");
      this.close();
    }));
  }
  onClose() {
...
var CreateServerModal = class extends import_obsidian2.Modal {
  constructor(app) {
    super(app);
  }
  onOpen() {
...
services:
  live-cursor-server:
    ...
    new import_obsidian2.Setting(contentEl).addButton((btn) => btn.setButtonText("Copy to Clipboard").setCta().onClick(() => {
...
var AdminConsoleModal = class extends import_obsidian2.Modal {
  constructor(app, plugin) {
    super(app);
    this.plugin = plugin;
  }
  async onOpen() {
    ...
    const fetchStats = async () => {
      try {
        ...
        const statusRes = await (0, import_obsidian2.requestUrl)({ url: statusUrl, method: "GET" });
        const usersRes = await (0, import_obsidian2.requestUrl)({ url: usersUrl, method: "GET" });
    ...
    let userTextComp;
    let passTextComp;
    new import_obsidian2.Setting(contentEl).setName("Username").addText((text2) => {
      userTextComp = text2;
      text2.onChange((v) => newUsername = v);
    });
    new import_obsidian2.Setting(contentEl).setName("Password").addText((text2) => {
      passTextComp = text2;
      text2.onChange((v) => newPassword = v);
    });
    new import_obsidian2.Setting(contentEl).addButton((btn) => btn.setButtonText("Create User").setCta().onClick(async () => {
      if (!newUsername || !newPassword) {
        new import_obsidian2.Notice("Please fill in both Username and Password fields.");
        return;
      }
      try {
        let httpUrl = this.plugin.settings.serverUrl.replace(/^ws/, "http").replace(/\/sync\/?$/, "");
        ...
        const res = await (0, import_obsidian2.requestUrl)({ url, method: "POST" });
        if (res.status === 200) {
          new import_obsidian2.Notice(`User "${newUsername}" created successfully!`);
          newUsername = "";
          newPassword = "";
          ...
          new import_obsidian2.Notice(`Failed to create user: ${res.text}`);
        }
      } catch (e) {
        new import_obsidian2.Notice(`Failed to create user: ${e.message}`);
      }
    }));
    this.timer = setInterval(fetchStats, 3e3);
  }
  onClose() {
    if (this.timer) {
...
var SetupWizardModal = class extends import_obsidian2.Modal {
  ...
  renderStep() {
    ...
    if (this.step === 1) {
      ...
      if (this.plugin.daemonProcess) {
        const okText = card.createEl("p", { text: "\u{1F7E2} Success! Server is running in the background." });
      ...
      new import_obsidian2.Setting(card).addButton((btn) => btn.setButtonText("Initialize Admin Registry").setCta().onClick(async () => {
        ...
        try {
          ...
          new import_obsidian2.Notice(`Failed to connect to local server: ${err.message}`);
        }
      }));
    } else if (this.step === 3) {
      ...
      card.createEl("h3", { text: "\u{1F389} Setup Complete!" });
      ...
      detailList.createEl("li", { text: "\u{1F7E2} Server: ws://localhost:1234/sync" });
      detailList.createEl("li", { text: `\u{1F7E2} Active Profile: ${this.plugin.settings.username}` });
      detailList.createEl("li", { text: "\u{1F7E2} Collaborator Cursor Color: Active" });
